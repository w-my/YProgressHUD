...
public extension YProgressHUD {
    ...
    class var imageSuccess: UIImage {
        get { shared.imageSuccess ?? UIImage.checkmark.withTintColor(UIColor.systemGreen, renderingMode: .alwaysOriginal) }
        set { shared.imageSuccess = newValue }
    }

    @available(iOS 13.0, *)
    class var imageError: UIImage {
        get { shared.imageError ?? UIImage.remove.withTintColor(UIColor.systemRed, renderingMode: .alwaysOriginal) }
        set { shared.imageError = newValue }
    }
}

// MARK: -
// MARK: - YProgressHUD extension
public extension YProgressHUD {
    ...
    class func show(_ status: String? = nil, interaction: Bool = false) {
        DispatchQueue.main.async {
            shared.setup(status: status, isToast: false, hide: false, interaction: interaction)
        }
    }
    ...
    class func show(_ status: String? = nil, icon animatedIcon: AnimatedIcon, interaction: Bool = true) {
        DispatchQueue.main.async {
            shared.setup(status: status, isToast: false, animatedIcon: animatedIcon, hide: true, interaction: interaction)
        }
    }
    ...
    class func showSucceed(_ status: String? = nil, interaction: Bool = true) {
        DispatchQueue.main.async {
            YProgressHUD.colorAnimation = UIColor.green.withAlphaComponent(0.68)
            shared.setup(status: status, isToast: false, animatedIcon: .succeed, hide: true, interaction: interaction)
        }
    }
    ...
    class func showFailed(_ status: String? = nil, interaction: Bool = true) {
        DispatchQueue.main.async {
            YProgressHUD.colorAnimation = UIColor.red.withAlphaComponent(0.68)
            shared.setup(status: status, isToast: false, animatedIcon: .failed, hide: true, interaction: interaction)
        }
    }
    ...
    class func showAdded(_ status: String? = nil, interaction: Bool = true) {
        DispatchQueue.main.async {
            YProgressHUD.colorAnimation = UIColor.white.withAlphaComponent(0.68)
            shared.setup(status: status, isToast: false, animatedIcon: .added, hide: true, interaction: interaction)
        }
    }
    ...
    class func show(_ status: String? = nil, icon: AlertIcon, interaction: Bool = true) {
        ...
        DispatchQueue.main.async {
            shared.setup(status: status, isToast: false, staticImage: image, hide: true, interaction: interaction)
        }
    }
    ...
    class func showSuccess(_ status: String? = nil, image: UIImage? = nil, interaction: Bool = true) {
        DispatchQueue.main.async {
            shared.setup(status: status, isToast: false, staticImage: image ?? YProgressHUD.imageSuccess, hide: true, interaction: interaction)
        }
    }
    ...
    class func showError(_ status: String? = nil, image: UIImage? = nil, interaction: Bool = true) {
        DispatchQueue.main.async {
            shared.setup(status: status, isToast: false, staticImage: image ?? YProgressHUD.imageError, hide: true, interaction: interaction)
        }
    }
    ...
    class func showProgress(_ progress: CGFloat, interaction: Bool = false) {
        DispatchQueue.main.async {
            shared.setup(isToast: false, progress: progress, hide: false, interaction: interaction)
        }
    }

    class func showProgress(_ status: String?, _ progress: CGFloat, interaction: Bool = false) {
        DispatchQueue.main.async {
            shared.setup(status: status, isToast: false, progress: progress, hide: false, interaction: interaction)
        }
    }
}
...
// MARK: -
// MARK: - YProgressHUD
public class YProgressHUD: UIView {
    ...
    private var colorStatus         = UIColor.white.withAlphaComponent(0.9)
    ...
    // MARK: - init
    static let shared: YProgressHUD = {
        let instance = YProgressHUD()
        return instance
    } ()
    ...
    // MARK: - setup
    private func setup(status: String? = nil, isToast: Bool = true, progress: CGFloat? = nil, animatedIcon: AnimatedIcon? = nil, staticImage: UIImage? = nil, delay: Double = -1, hide: Bool, interaction: Bool) {
        ...
        if !isToast && (progress == nil) && (animatedIcon == nil) && (staticImage == nil) { setupAnimation()                }
        if !isToast && (progress != nil) && (animatedIcon == nil) && (staticImage == nil) { setupProgress(progress)         }
        if !isToast && (progress == nil) && (animatedIcon != nil) && (staticImage == nil) { setupAnimatedIcon(animatedIcon) }
        if !isToast && (progress == nil) && (animatedIcon == nil) && (staticImage != nil) { setupStaticImage(staticImage)   }
        ...
        if (hide) {
            ...
            let time = delay > 0 ? delay : Double(text.count) * 0.03 + 1.25
    ...
    private func setupBackground(_ interaction: Bool) {
        if (viewBackground == nil) {
            ...
            guard let mainWindow = getKeyWindow() else { return }
    ...
    // MARK: - frame
    private func setupSize(_ isToast: Bool) {
        var width: CGFloat = 120
        var height: CGFloat = isToast ? 36 : 120
        ...
        if let text = labelStatus?.text {
            ...
            height = ceil(rectLabel.size.height) + height

            if (width < 120) { width = 120 }

            rectLabel.origin.x = (width - rectLabel.size.width) / 2
            rectLabel.origin.y = (height - rectLabel.size.height) / 2 + (isToast ? 0 : 45)
    ...
    private func hudShow() {
        ...
        if (self.alpha != 1) {
            self.alpha = 1
            toolbarHUD?.alpha = 0
            toolbarHUD?.transform = CGAffineTransform(scaleX: 0.5, y: 0.5)

            UIView.animate(withDuration: 0.15, delay: 0, options: [.allowUserInteraction, .curveEaseIn], animations: {
                self.toolbarHUD?.transform = CGAffineTransform(scaleX: 0.7, y: 0.7)
